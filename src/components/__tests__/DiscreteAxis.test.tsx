...
describe('Discrete Axis test', () => {
  ...
  test('it should compute axis line coordinates for bottom axis', () => {
    setup(props)
    expect(screen.getByTestId('discrete-axis')).toHaveAttribute(
      'x1',
      '0'
    );
    expect(screen.getByTestId('discrete-axis')).toHaveAttribute(
  ...
  test('it should compute axis line coordinates for top axis', () => {
   ...
   test('it should not format tick text if it\'s less than 10 characters', () => {
    setup(props)
    expect(screen.queryAllByTestId('d3reactor-ticktext')[0]).toHaveTextContent('2019-07-22');
  });
  ...
  test('it should format tick text if it\'s more than 10 characters and represents date', () => {
    ...
    setup(updatedProps)
    expect(screen.queryAllByTestId('d3reactor-ticktext')[0]).toHaveTextContent('2/1/2000');
    expect(screen.queryAllByTestId('d3reactor-ticktext')[0]).not.toHaveTextContent('2000-02-01T00:00:00.000Z');
  });
  ...
  test('it should rotate tick text on bottom axis if text width is more than width of rectangles', () => {
    ...
    expect(elements[0]).toHaveAttribute('transform', "translate(51.03086419753081, 661), rotate(-90)");
    expect(elements[2]).toHaveAttribute('transform', 'translate(223.87037037037032, 661), rotate(-90)');
  })
  ...
  test('it should correctly position tick text on bottom axis', () => {
    ...
    const elements = screen.queryAllByTestId('d3reactor-ticktext');
    expect(elements[0]).toHaveAttribute('transform', "translate(74.69135802469125, 1080)");
    expect(elements[4]).toHaveAttribute('transform', "translate(617.9012345679012, 1080)");
  })
  ...
  test('it should correctly position tick text on top axis', () => {
    ...
    const elements = screen.queryAllByTestId('d3reactor-ticktext');
    expect(elements[0]).toHaveAttribute('transform', "translate(74.69135802469125, -7)");
    expect(elements[4]).toHaveAttribute('transform', "translate(617.9012345679012, -7)");
  });
  ...
  setup(updatedProps);
  expect(screen.queryByText('Bethesda-Rockville-Frederick, MD Met Div')).toBeInTheDocument();
  cleanup();
  ...
  expect(screen.queryByText('Bethesda-Rockville-Frederick, MD Met Div')).not.toBeInTheDocument();
