...
const initialProps: ContinuousAxisProps = {
  ...
  const leftProps:ContinuousAxisProps = {...initialProps, type: 'left', y:0, scale: yScale, xTicksValue: undefined, margin:{
      "left": 80,
      "right": 20,
      "top": 20,
      "bottom": 40
...
describe('Continuous Axis test', () => {
  ...
  test('it should not render a line for vertical axis if not ScatterPlot', () => {
    setup(leftProps);
    expect(screen.queryByTestId('d3reactor-continuous')).not.toBeInTheDocument();
  });
  ...
  test('it should compute axis line coordinates according to type input', () => {
    setup(initialProps);
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('x1', '0');
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('x2', '400'); 
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('y1', '400'); 
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('y2', '400'); 
    cleanup();
    const topProps:ContinuousAxisProps = {...initialProps, type: 'top', y:0, margin:{
      "left": 80,
      "right": 20,
    ...
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('x1', '0');
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('x2', '400'); 
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('y1', '0'); 
    expect(screen.getByTestId('d3reactor-continuous')).toHaveAttribute('y2', '0'); 
  ...
  test('it should render formatted date tick text', () => {
    ...
    expect(screen.queryByText( "2019-10-01T04:00:00.000Z")).not.toBeInTheDocument(); 
  })
  ...
  test('it should filter tick text elements number based on height for vertical axes', () => {
    ...
    .range([940, 0])
    .nice();
    const leftPropsResized = {...leftProps, width: 1000, height: 1000, scale:yScaleUpdated};
  ...
  test('it should display gridlines when true', async () => {
    ...
    cleanup();
    setup({...initialProps, type: 'top', y:0, margin:{
      "left": 80,
      "right": 20,
      "top": 80,
      "bottom": 20}, xGrid:true});
    expect(screen.queryAllByTestId('d3reactor-gridline')).toHaveLength(10);
    elements = screen.queryAllByTestId('d3reactor-gridline');
    expect(elements[0]).toHaveAttribute('x1', '0');
    expect(elements[0]).toHaveAttribute('x2', '0');
